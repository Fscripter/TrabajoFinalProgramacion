...
function rotateImg(img, rotation, emptyBuffer, existentBuffer) {
...
function GetPositionFromRotation(coodernates, degrees) {
    let radians = (Math.PI * 2 * degrees) / 360;
    let rotationMatrix = [
        [Math.cos(radians), -Math.sin(radians)],
        [Math.sin(radians), Math.cos(radians)],
    ];
    let a = math.multiply(math.matrix(rotationMatrix), math.matrix([[coodernates.x], [coodernates.y]]));
    return a;
}
function getMirrorPosition(coodernates, espejo) {
    let x = 1;
    let y = 1;
    if (espejo.x) {
        x = -1;
...
function mirrorImg(img, espejo, emptyBuffer, existentBuffer) {
    for (let y = 0; y < img.height; y++) {
        for (let x = 0; x < img.width; x++) {
            //Recorro la imagen
            let position = { x, y };
...
function LinearImg(img, rotacion = 0, espejo, space) {
